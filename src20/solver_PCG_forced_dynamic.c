 ...
 *********/
	...
	*/
	 ...
	 **************************/
		...
		#pragma omp for reduction (+:BNRM2) schedule(dynamic)
		for(i=0; i<N; i++) {
		  BNRM2 += B[i]*B[i];
		}
	 ...
	 ****************/
			...
			#pragma omp for reduction(+:RHO) schedule(dynamic)
			for(i=0; i<N; i++) {
			  RHO += W[R][i] * W[Z][i];
			}
	 ...
	 ************************/
			...
			#pragma omp for reduction(+:C1) schedule(dynamic)
			for(i=0; i<N; i++) {
				C1 += W[P][i] * W[Q][i];
			}
			#pragma omp single
	 ...
	 ***************************/
			...
			DNRM2 = 0.0;
			#pragma omp barrier
			#pragma omp for reduction(+:DNRM2) schedule(dynamic)
			for(i=0; i<N; i++) {
			  DNRM2 += W[R][i]*W[R][i];
			}
