 ...
 * {r0} = {b} - {A}{xini} *
 **************************/
#pragma omp parallel for private (i,VAL,j) schedule(dynamic)
	for(i=0; i<N; i++) {
		VAL = D[i] * X[i];
	...
	BNRM2 = 0.0;
#pragma omp parallel for private (i) reduction (+:BNRM2) schedule(dynamic)
	for(i=0; i<N; i++) {
	  BNRM2 += B[i]*B[i];
	}
 ...
 ****************/
		RHO = 0.0;
#pragma omp parallel for private (i) reduction(+:RHO) schedule(dynamic)
		for(i=0; i<N; i++) {
		  RHO += W[R][i] * W[Z][i];
		}
 ...
 ************************/
		C1 = 0.0;
#pragma omp parallel for private (i) reduction(+:C1) schedule(dynamic)
		for(i=0; i<N; i++) {
			C1 += W[P][i] * W[Q][i];
		}
...
#pragma omp parallel for private (i) schedule(dynamic)
		...
		DNRM2 = 0.0;
#pragma omp parallel for private (i) reduction(+:DNRM2) schedule(dynamic)
		for(i=0; i<N; i++) {
		  DNRM2 += W[R][i]*W[R][i];
		}
